...
export function BackgroundPage({ activeSection, isNavCollapsed, onNavigate, onToggleNavigation, publicView = false }: BackgroundPageProps) {
  const { user } = useAuth();
  const [background, setBackground] = useState<BackgroundSummary | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (!publicView && !user?.access_token) {
      return;
    }

    setError(null);

    const headers = publicView ? undefined : { Authorization: `Bearer ${user?.access_token}` };
    const endpoint = publicView ? `${apiBaseUrl}/api/background/sean/public` : `${apiBaseUrl}/api/background/sean`;
    ...
    <Box className="backgroundPage">
      <Box className="backgroundHero">
        ...
        <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
          {publicView ? (
            <Chip className="entitlementChip" label="Public Resume" />
          ) : (
  ...
  if (publicView) {
    return <PublicResumeShell>{content}</PublicResumeShell>;
  }

  return (
    <DashboardLayout
      activeSection={activeSection ?? "background"}
      isNavCollapsed={Boolean(isNavCollapsed)}
      onNavigate={onNavigate ?? (() => undefined)}
      onToggleNavigation={onToggleNavigation ?? (() => undefined)}
    >
      {content}
    </DashboardLayout>
  );
